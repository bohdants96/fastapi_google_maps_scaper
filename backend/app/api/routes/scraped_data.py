...
from app.models import ScrapedData, ScrapedDataCreate, ScrapedDataBase, ScrapedDataPublic, ScrapedDatasPublic, Location, Country
...
def download_csv(
    ...
    writer.writerow(["id", "company_name", "business_type", "company_address", "company_phone", "country_id", "location_id", "state", "zip_code", "created_at"])

    for data in scraped_datas:
        writer.writerow([data.id, data.company_name, data.business_type, data.company_address, data.company_phone, data.country_id, data.location_id, data.state, data.zip_code, data.created_at])
